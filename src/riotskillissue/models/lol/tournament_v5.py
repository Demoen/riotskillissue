...
class TournamentCodeUpdateParametersV5(BaseModel):
    ...
    map_type: Literal["SUMMONERS_RIFT", "HOWLING_ABYSS"] = Field(
        alias="mapType",
        description="".join(
            ("The map type\n             (Legal values:  SUMM", "ONERS_RIFT,  HOWLING_ABYSS)")
        ),
    )
    pick_type: Literal["BLIND_PICK", "DRAFT_MODE", "ALL_RANDOM", "TOURNAMENT_DRAFT"] = Field(
        alias="pickType",
        description="".join(
            (
                "The pick type\n             (Legal values:  BLI",
                "ND_PICK,  DRAFT_MODE,  ALL_RANDOM,  TOURNAMENT",
                "_DRAFT)",
            )
        ),
    )
...
class TournamentCodeV5(BaseModel):
    ...
    spectators: str = Field(
        alias="spectators",
        description="".join(("The spectator mode for the tournament code gam", "e.")),
    )
    team_size: int = Field(
        alias="teamSize",
...
class TournamentGamesV5(BaseModel):
    ...
    losing_team: List[TournamentTeamV5] = Field(
        alias="losingTeam",
    )
    ...
    winning_team: List[TournamentTeamV5] = Field(
        alias="winningTeam",
    )
